...
<section class="container">
    <div class="themeborder">
        ...
        if(isset($_SESSION['priv'])){
            if($_SESSION['priv']==1){
                echo "<a href='addTheme.html'>Добавить</a>";
            }
        }
        ?>
    </div>
    <div class="themes">
        ...
        $queryString="SELECT `themes`.`Nazvanie`,`themes`.`Opisanie` FROM
`themes` join `razdel` on `themes`.`ID_Razdel`=`razdel`.`ID_Razdel` and `razdel`.`Nazvanie`='$r'";
        $query=mysqli_query($con,$queryString);
        ...
        if(!empty($query))while ($zap=mysqli_fetch_assoc($query)){
            ...
            ?>
            <div class="thema">
                <a href=<?php echo "obs.php?thema=$razdName"; ?>>
